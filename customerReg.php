...
if(isset($_POST['submit']))
{
	if(isset($_GET['editid']))
	{
		$sql ="UPDATE customer SET  `customer_name`='$_POST[customername]', `address`='$_POST[address]', `state_id`='$_POST[state]', `city_id`='$_POST[city]', `pincode`='$_POST[pincode]', `contact_no`='$_POST[cntctnum]', `email_id`='$_POST[email_id]', `password`='$_POST[password]', `customer_type`='$_POST[customertype]', `status`='$_POST[status]' WHERE customer_id='$_GET[editid]'";
	...
	{
		$sql="INSERT INTO `customer`( `customer_name`, `address`, `state_id`, `city_id`, `pincode`, `contact_no`, `email_id`, `password`, `customer_type`, `status`) VALUES ('$_POST[customername]','$_POST[address]','$_POST[state]','$_POST[city]','$_POST[pincode]','$_POST[cntctnum]','$_POST[email_id]','$_POST[password]','$_POST[customertype]','$_POST[status]')";	
		if(!mysqli_query($con,$sql))
		{
...
<div class="text-center">
	<button type="submit" name="submit" id="submit" class="btn btn-info btn-lg px-5 " >Click here to Register</button>
</div>
</form>
                </div>
              </div>
...
var emailExp = /^[\w\-\.\+]+\@[a-zA-Z0-9\.\-]+\.[a-zA-z0-9]{2,4}$/; //Variable to validate Email ID
	...
	else if(document.frmcstreg.password.value.length < 5)
	{
